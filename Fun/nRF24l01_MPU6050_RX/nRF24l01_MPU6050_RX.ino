...
#define MISO_pin 12
#define SCK_pin 13 
#define packet_size 3 //number of bytes getting sent each time
...
void NRF_transmit(byte x, byte y, byte sign){
  ...
  data_in[1] = SPI.transfer(x);
  data_in[2] = SPI.transfer(y);
  data_in[3] = SPI.transfer(sign);
