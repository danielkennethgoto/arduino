...
#define MISO_pin 14
#define SCK_pin 15
#define packet_size 4 //number of bytes getting sent each time
...
void setup() {
  ...
  set_PipeLength(0, 5);//set_PipeLength(pipe 0-5,bytes)
...
void NRF_transmit(byte x, byte y, byte sign){
  ...
  data_in[2] = SPI.transfer(x);
  data_in[3] = SPI.transfer(y);
  data_in[4] = SPI.transfer(sign);
  digitalWrite(CSN_pin, HIGH);
